...
$('.jsSelectItems').each(function(){
	...
	var $answer = _this.find('.jsAnswer'),
		...
		$resetKeepCorrect = _this.find('.jsResetKeepCorrect'),
		$tryAgain = _this.find('.jsTryAgain'),
		$nextQuestion = _this.find('.jsNextQuestion'),

		scrollAnchor = _this.data('scroll-anchor'),

		maxSelections = _this.data('max-selected'),
		questionType = _this.data('question-type'),
	...
	$answer.on('click', function(e){
		...
		if(maxSelections === 1){
			...
			if(questionType === "true-or-false"){
				$(this).parents('.question-wrapper').find('.selected').removeClass('selected');
	...
	$check.on('click', function(e){
		...
		if(minimumSelected.length > 0 || _this.find('.selected').length > 0){
	...
	// Reset - remove all selected answers.
	$resetKeepCorrect.on('click', function(e){

		e.preventDefault();
		$check.removeClass('visibility-hidden');
		
		var $wrong = _this.find($('.jsAnswer.selected:not(.jsCorrect)'));
	...
	$nextQuestion.on('click', function(e){
		...
		_this.next(".jsSelectItems").find('.jsQuestion').removeClass('hide');
		_this.parents('.jsSelectItems').next('.jsSelectItems').find('.jsQuestion').removeClass('hide');
