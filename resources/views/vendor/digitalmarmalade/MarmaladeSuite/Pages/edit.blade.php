...
{!! Cropper::header() !!}
{!! inlineAsset('/vendor/digitalmarmalade/MarmaladeSuite/PageBuilder/Animator/js/animator.js') !!}
{!! inlineAsset('/vendor/digitalmarmalade/MarmaladeSuite/PageBuilder/Animator/css/animator.css') !!}
@endsection
...
<nav class="add-a-widget">
    ...
    <ul>
        ...
        <li>				
            <a class="js-pagebuilder-add-item-button" href="{{ route('msuiteAddPageItem', ['pageId' => $page->getId(), 'itemId' => $itemId]) }}">
                {{ $item['name'] }}
            </a>
        </li>
        @endif
...
<form action="{{ route('msuitePageUpdate', ['pageId' => $page->getId()]) }}" method="post" enctype="multipart/form-data" class="main-form pagebuilder-form">
    {!! csrf_field() !!}
    ...
    ($(function() {
        ...
        $('.sortable-panels').on('click', '.js-form-panel-move-up-button', function(event) {
            ...
            if (previousPanel.length) {
                animator.setDuration(1).swapUp(panel, previousPanel, function() {
            ...
            if (nextPanel.length) {
                animator.setDuration(1).swapDown(panel, nextPanel, function() {
       ...
       $('body').on('click', '.js-pagebuilder-add-item-button', function(event) {
           ...
           $.get($(this).attr('href'), function(response) {
                ...
                animator.setDuration(1).add(newItem);
           });
       });
       $('.js-pagebuilder-items-wrapper').on('click', '.js-form-panel-delete-button', function(event) {
            ...
            var self = this;
            $.get($(this).attr('href'), function(response) {
                animator.setDuration(0.5).delete($(self).closest('.js-pagebuilder-item-wrapper'), function() {
